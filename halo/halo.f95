!This is the parallel program for SHPC4002 Astro Assignment 2 (MPI+OpenMP)
!
!by Mitchell Cavanagh, 21727193
!
!compile with $ mpifort -fopenmp -o halo halo.f95
!
!run with $ mpirun -np # ./halo
!NB: {num_cells} must be divisible by #
!
!ver 0.4b, October 24, 2018
program halo
	...
	!----------------------------------------------------------------------------
	!The number of threads for the program (caution when setting >2)
	num_threads = 2
	...
	if (mpi_rank == mpi_rootp) then
		write(*,*)'This is halo (MPI+OpenMP), executing with',mpi_size,'processes'
