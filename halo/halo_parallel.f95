!This is the parallel program for SHPC4002 Astro Assignment 1
!
!by Mitchell Cavanagh, 21727193
!
!compile with $ gfortran -fopenmp -o halo_parallel halo_parallel.f95
!
!ver 0.5.1, September 14, 2018
program halo_parallel
	...
	INTEGER :: i, j, k, np, npart, num_cells, num_neighbours, num_threads, tid
	TYPE(Mesh_t), DIMENSION(:,:,:), ALLOCATABLE :: mesh
	TYPE(Mesh_t), DIMENSION(:), ALLOCATABLE :: neighbours
	
	
	!----------------------------------------------------------------------------
	!The number of threads for the program
	num_threads = 4
	...
	subroutine calculate_density(h_smooth, w_local, neighbours)
		...
		!$OMP PARALLEL DO PRIVATE(r_max, w, ll, a_pos, b_pos, r, h)
	...
	subroutine check_neighbours(mesh, neighbours)
		...
		!$OMP PARALLEL !extra parallel region required to output correctly
		!$OMP PARALLEL DO PRIVATE(c, ll, distances, current, a_pos, b_pos, d_min, d_max, d_num, d, l, m) COLLAPSE(3)
		do k = 1,num_cells,1
			do j = 1,num_cells,1
				do i = 1,num_cells,1
					!$OMP FLUSH (mesh, neighbours)
	...
	subroutine init_mesh(mesh)
		...
		!$OMP PARALLEL DO PRIVATE(new_ll) COLLAPSE(3)
		do k = 1,num_cells,1
			do j = 1,num_cells,1
				do i = 1,num_cells,1
